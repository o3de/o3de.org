...
**Open 3D Engine (O3DE)** uses [CTest](https://cmake.org/cmake/help/latest/manual/ctest.1.html) to drive all testing across the engine. Python is used for automating the testing of the engine.

A Python automated test uses two Python interpreter instances:

* **External Python interpreter**: The test driver for the test suite. The external interpreter is responsible for launching the Editor and providing the tests to run to the embedded interpreter. This instance uses the [pytest](https://docs.pytest.org/) framework and LyTestTools for specifying the tests. LyTestTools is a set of Python tools and utilities that have been written around pytest.
* **Embedded editor Python interpreter** : An interpreter instance inside the Editor with bindings to drive Editor activities and get feedback. This instance is responsible for performing the actual test and is part of the `EditorPythonBindings` Gem.
...
* **CMake configuration file**: For adding a Python Test Suite into the CI pipeline, it must be registered with CTest via a `CMakeLists.txt` file.
...
* **tests/** - The `tests` directory contains the tests themselves that are run with the Editor's Python embedded interpreter. Test names must follow the pattern `<FeatureName>_<TestThatDoesSomething>.py format. Anything that is not supposed to be a test (like utilities) must follow `UpperCamelCase`, as in the following example.
...
For writing editor automated tests, it is strongly recommended to use the **editor_test.py** utility in offered as part of the `LyTestTools`, located at `o3de/Tools/LyTestTools/ly_test_tools/o3de/editor_test.py`.
...
Each test suite requires a main suite class which inherits from `EditorTestSuite`, contained in the `Tools/LyTestTools/ly_test_tools/o3de/editor_test.py` directory. Inside the suite's main class, you declare subclasses inheriting from `EditorSingleTest` and load test modules from the individual tests to run in the Editor embedded interpreter.
...
| `timeout_editor_shared_test` | Maximum time that one Editor can take to run multiple tests, in seconds. | `180` |
| `get_number_parallel_editors()` | Overridable function that returns the maximum number of editors to run at the same time. | `lambda: 8` |
...
| `setup()` | Custom callback that will be called before the Editor is launched and loads the test. | `None` |
| `teardown()` | Custom callback that will be called after the Editor finishes running the test. | `None` |
| `wrap_run()` | This function wraps the runtime of the test. All the code before yielding will run before launching the editor, and the code after yielding will run after closing the Editor. | `None` |
...
The next step is writing the tests which will run in the Editor. The Editor's internal Python runtime is started and loads a test specified as a command-line argument.
...
A full example of how to structure a test that runs in the Editor:
...
The tests which run in the Editor have support for **batched** and **parallel** tests. Batched tests are collections of tests which can run together in a single Editor, and parallel tests launch multiple editors. A test can be both part of a batch, and run in parallel.

### Batched tests

In batched mode, one single Editor is runs a batch of tests. Batching tests reduces the time taken to launch and close Editors to run tests.

![Batched Tests](/images/user-guide/testing/parallel-pattern/batched-tests.png)

### Parallel tests

In parallel mode, multiple Editors launch at once to run one test each. The runner of the tests can set the maximum number of allowed Editors, where when one Editor closes if the maximum is reached, a new Editor is launched.

![Parallel tests](/images/user-guide/testing/parallel-pattern/parallelization.png)

### Parallel Batched tests

Tests can also be run in parallel batches, also called **shared tests**. Shared tests should be used when tests can run in the same Editor without relying on effects of any other test (batched mode) and with multiple independent editors running their each individual batch (parallel mode). When possible, tests should be made as shared tests in order to take advantage of the performance improvements offered by maximizing parallelization.

![Parallel And Batch](images/user-guide/testing/parallel-pattern/parallel-and-batch.png)

{{ note }}
Which tests are batched together in which Editor instance is arbitrary in this mode.
{{ /note }}
...
In order to use any of these modes, the only needed change is to modify the class which loads and runs each test. Use the `EditorBatchedTest`, `EditorParallelTest`, and `EditorSharedTest` superclasses in place of `EditorSingleTest`.
...
Examples of tests which can be batched, but not parallel is a physics test that makes a modification to the project wide material library at the start and restores to the default setting when finishing. By changing the state of a resource shared between Editors (disk storage), it could cause parallel tests to fail.
...
* `--no-editor-batch`: Don't batch tests, All tests run in a single editor. Shared tests will become parallel, or if parallel tests are also disabled, become serial.
...
* **Tests should be be self-contained**. No external tools should determine if the test has passed or failed. Instead, the Editor should do this work. Use the one of the Editor return codes as result of the test, `0x0` for success and `0xF` for failure.
* **Don’t read the Editor log file directly**. To read Editor log information else use the `DebugTraceBus` and `Tracer` utilities. The `DebugTraceBus` is able to capture any of these, without having to create an I/O dependency on a file.
...
```
* **Tests must be deterministic **. The application will run at different FPS and take different amounts of time time loading assets. When running the test make sure it is deterministic with these conditions.
* **Wait on conditions instead of waiting for time.** Some tests require performing an action and waiting for completion. To optimize the amount of time your test runs, and avoiding false negatives caused by timeouts, wait on a condition instead of for an arbitrary time:
    ```python
### BAD ###
enable_gravity()
...
During test development, it can be time-consuming to launch the Editor on a test-by-test basis. Instead, you can launch the Editor normally and then use the `pyRunFile` command from the Editor console:

```cmd
pyRunFile ../../Gem/PythonTests/<MyFeature>/tests/<MyFeature>_<TestName>.py
```

To attach a debugger to a running test, use one of the following utilities:you can use the following utilities:

* **Wait for debugger**: Use the command-line argument `--wait-for-debugger` when launching an Editor to run the test. This argument pauses the Editor at the earliest moment of execution and resumes at the moment a debugger is attached. This behavior can also be controlled with the  `wait_for_debugger` variable in an Editor test specification, or forced by calling `general.wait_for_debugger()`.
