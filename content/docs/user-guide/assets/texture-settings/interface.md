...
With **Texture Settings**, you can specify how image source assets are processed for **Open 3D Engine (O3DE)**. Texture Settings operates on one image source asset at a time. The options you choose in Texture Settings are saved in a `.assetinfo` sidecar file for the image source asset. **Asset Processor** reads the `.assetinfo` file and applies the options whenever the image source asset is processed.

To open Texture settings, do the following:

1. In **O3DE Editor**, in **Asset Browser**, select the image source asset to modify. You can use the search bar at the top of Asset Browser to filter the list.

1. **Right-click** the image source asset and then choose **Edit Texture Settings...**.

![ Right click an asset to open Texture Settings. ]( /images/user-guide/assets/texture-settings/open-texture-settings.png )

## Interface sections

The Texture Settings interface has three sections: a texture preview, texture settings, and mipmap settings. Buttons that **Apply** the settings, and **Close** Texture Settings are located at the bottom of the Texture Settings window.

### Texture preview

The texture preview area displays the image product asset and information and has a few tools that you can use to examine 
