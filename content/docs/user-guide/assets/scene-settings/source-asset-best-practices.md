...
* **Root bone** - A root bone should be added you the actor's skeleton. It is required for root motion extraction. For more information check the [Data Driven Root Motion](/docs/learning-guide/tutorials/animation/data-driven-root-motion) tutorial topic.
...
* **Bone names** - The same advice for naming mesh nodes applies here. Names should be alpha-numeric strings. Use underscore (`_`) as a separator if needed. Using special characters or dot (`.`) as a separator can cause issues when processing. It is also recommend that you use sensible bone names. In some places, such as [Scene Settings](./scene-settings), you might need to select specific bones from the scene source asset. It is much easier to find the bones in the list if they are sensibly named.
...
* **Orientation** - Orient the actor so that it is facing down the positive view axis (the Y axis).
