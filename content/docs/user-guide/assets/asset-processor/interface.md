...
The Asset Status list can be filtered by entering keywords and regular expressions in the filter box. The regular expressions (regex) are standard `std::regex` in extended format. The standard `std::regex` rules apply.

The following is an example regex to search for all files ending with .png:
...
The following is an example regex to search for any files under a `Slices` subdirectory:
...
* {{< icon "warning-yellow.svg" >}} **Warning** - See the Message column for information about a potential processing issue for this asset.
