...
Files aren't assets unless a builder exists that processes them, so the next table in this process is the **BuidlerInfo** table. This table tracks information related to [Asset Builders.](docs/user-guide/assets/pipeline/asset-builders) This has the BuilderID, Guid, and analysis fingerprint of each builder. The file matching patterns are not tracked in the database because they're defined in the code and don't need to be tracked here, however you can best view them in the Builder tab of the Asset Processor. The analysis fingerprint is used by the Asset Processor to know when a builder changes, so it can re-process all assets matching that builder.
...
The **Jobs** table contains all jobs that have been completed. Jobs are associated with Asset Builders, you can find more information [here.](docs/user-guide/assets/pipeline/asset-builders#process-job) SourcePK is the SourceID from the Sources table for the source asset processed, JobKey is a descriptive string registered in CreateJobs for that job. Platform is the intended deployment target for the asset, such as pc or Android. BuilderGuid is the unique identifier for the builder used for the job. Status is the result code of the job, 2 means failed, 3 is failed because the source name was longer than the maximum length, 4 means Completed, and 5 is a failure due to the source asset being missing. The job run key is the order the job was created. The number of errors and warnings that occured in each job is tracked in this table, as well.
...
# Updating Asset Processor & the Asset Database
...
1. The next step is to register the query object with the system. The query object was defined in step 1.3 in the previous section. Most of these are grouped by table.  imply add a call to AddStatement.
   * Example:
```
...
1. Add a query function to make the query usable.
   ...
   * There are already pre-made handler definitions for each existing table, as well as a combinedHandler for queries that work with all the source/job/product/scanfolder table data. For the most part these should be pretty simple as there are already result handler functions for the typical use cases.
   * Example:
```
