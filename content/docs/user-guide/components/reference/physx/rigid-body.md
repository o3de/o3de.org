...
| **Angular damping** | Sets the rate of decay over time for angular velocity even if no forces are acting on the rigid body. A non-zero value eventually stops the rigid body if no torque force is applied. Value must be non-negative. | Float: 0 to Infinity | `0.15` |
| **Sleep threshold** | Sets the kinetic energy per unit mass below which the rigid body can go to sleep. Value must be non-negative. | Float: 0 to Infinity | `0.005` |
...
| **Min advance coefficient** | Fine tune continuous collision detection. Lower values reduce clipping but can affect motion smoothness. | Float: 0.01 and 0.99 | `0.15` |
...
| **Mass** | If **Compute Mass** is disabled, a **Mass** value can be specified for the PhysX rigid body in kilograms. | Float: 0 to Infinity | `1.0` |
...
| **Inertia diagonal** | If **Compute inertia** is disabled, an **Inertia diagonal** can be specified as the diagonal elements of the inertia tensor. This is the torque required to rotate the rigid body on each axis. | Vector3 | X: `1.0`, Y: `1.0`, Z: `1.0` |
| **Maximum angular velocity** | Clamps angular velocity to the specified value. This is useful in scenarios where the rigid body rotates at an unrealistically fast angular velocity. | `0` to `Infinity` | `100.0` |
