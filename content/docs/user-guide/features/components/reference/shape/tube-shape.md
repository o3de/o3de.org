...
| **Edit** | Click to enter Edit mode. In Edit mode, you can modify the Variable Radius for each point of the Tube Shape in the Perspective view by clicking and dragging the black handles at each spline point. While in Edt mode, the Edit menu in the menu bar displays available actions and hotkeys. To exit Edit mode, choose **Done** in the component interface. |  |  |
