...
To create a data overlay, you implement an [EBus](/docs/user-guide/features/engine/ebus/_index.md) through which the data is serialized\. The following example is the code that implements unit testing for the data overlay feature \(`\dev\Code\Framework\AzCore\Tests\Serialization.cpp`\):
