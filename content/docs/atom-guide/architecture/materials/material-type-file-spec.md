...
* **groups**: A list of property groups that will appear in the Inspector window of the Material Editor. Each group contains the following:
  * **id**: An identifier for this group that is unique to this material type. The value must be formatted in C-style (start with an alpha character and contains only alpha, numeric, and underscore characters). 
...
### **functor**
An array of data that is sent into functors, which were defined in the C++ or Lua. Each functor data contains the following: 
* **type**: The name of the functor. This name corresponds to the defined functor in the C++ or Lua script. 
