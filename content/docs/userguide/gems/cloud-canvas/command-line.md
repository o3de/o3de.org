---
description: ' Use the Cloud Canvas lmbr_aws.cmd command line tool for working with AWS
  resources. '
title: Using the Cloud Canvas Command Line
---
# Using the Cloud Canvas Command Line {#cloud-canvas-command-line}
...
## Syntax {#cloud-canvas-command-line-syntax}
...
## Configuration {#cloud-canvas-command-line-configuration}
...
### Environment Variables {#cloud-canvas-command-line-configuration-evars}
...
### `cloud-gem` Commands {#cloud-canvas-command-line-cloud-gem-commands}

The `lmbr_aws cloud-gem` commands create, enable, and disable cloud gems\.

#### cloud\-gem create {#cloud-canvas-command-line-cloud-gem-create}
...
#### cloud\-gem enable {#cloud-canvas-command-line-cloud-gem-enable}
...
#### cloud\-gem\-tts import\-tts\-zip {#cloud-canvas-command-line-cloud-gem-tts}
...
### `deployment` Commands {#cloud-canvas-command-line-deployment-commands}
...
#### deployment default {#cloud-canvas-command-line-deployment-default}
...
#### deployment list\-resources {#cloud-canvas-command-line-deployment-list-resources}
...
#### deployment protect {#cloud-canvas-command-line-deployment-protect}
...
#### deployment tags {#cloud-canvas-command-line-deployment-tags}
...
#### deployment update\-access {#cloud-canvas-command-line-deployment-update-access}
...
#### deployment update, deployment upload, deployment upload\-resources {#cloud-canvas-command-line-deployment-upload}
...
### `function` Commands {#cloud-canvas-command-line-function-commands}

The `lmbr_aws function` commands manage folders, CloudWatch logs, and code for Lambda functions\.

#### function create\-folder {#cloud-canvas-command-line-function-create-folder}
...
#### function get\-log {#cloud-canvas-command-line-function-get-log}
...
#### function upload\-code {#cloud-canvas-command-line-function-upload-code}
...
### `login-provider` Commands {#cloud-canvas-command-line-login-provider-commands}
...
#### login\-provider add {#cloud-canvas-command-line-login-provider-add}
...
#### mappings list {#cloud-canvas-command-line-mappings-list}
...
#### mappings update {#cloud-canvas-command-line-mappings-update}
...
#### parameter list {#cloud-canvas-command-line-parameter-list}
...
#### parameter set {#cloud-canvas-command-line-parameter-set}
...
### `profile` Commands {#cloud-canvas-command-line-profile-commands}

The `lmbr_aws profile` commands manage the AWS profiles that you use for Cloud Canvas\.

#### profile add {#cloud-canvas-command-line-profile-add}
...
#### profile default {#cloud-canvas-command-line-profile-default}

Set, clear, or show the default AWS profile\. The `profile default` command modifies the `DefaultProfile` section of the `[user\-settings\.json](/docs/userguide/gems/cloud-canvas/resource-definitions#cloud-canvas-user-settings)` file\. As of Lumberyard 1\.15, the [Cloud Canvas cleanup tool](/docs/userguide/gems/cloud-canvas/administration-aws-resource-cleanup#cloud-canvas-administration-aws-resource-cleanup-tool) uses this setting to determine the AWS profile to use\.
...
#### profile list {#cloud-canvas-command-line-profile-list}

List the AWS profiles that have been configured\.

#### profile remove {#cloud-canvas-command-line-profile-remove}
...
#### profile rename {#cloud-canvas-command-line-profile-rename}
...
#### profile update {#cloud-canvas-command-line-profile-update}
...
#### project create {#cloud-canvas-command-line-project-create}
...
##### How `project create` Works {#cloud-canvas-command-line-project-create-details}

The `project create` command performs the following tasks:

****

1. Creates the project's AWS CloudFormation stack using a bootstrap template that defines only the [Configuration Bucket](/docs/userguide/gems/cloud-canvas/resource-deployments#cloud-canvas-configuration-bucket) resource\.

1. Uploads the [project\-template\.json](/docs/userguide/gems/cloud-canvas/resource-definitions#cloud-canvas-project-template) file and the zipped up contents of the [project\-code subdirectory](/docs/userguide/gems/cloud-canvas/resource-definitions#cloud-canvas-project-code-subdirectory) to the [Configuration Bucket](/docs/userguide/gems/cloud-canvas/resource-deployments#cloud-canvas-configuration-bucket)\.
...
#### project delete {#cloud-canvas-command-line-project-delete}
...
#### project update, project upload {#cloud-canvas-command-line-project-upload}
...
##### How `project update` works {#cloud-canvas-command-line-project-upload-details}
...
For more information, see [ Applying Framework Updates to a Project](/docs/userguide/gems/cloud-canvas/cgf-rm-versioning#cloud-canvas-cgf-rm-versioning-applying-framework-updates)\.

### `resource-group` Commands {#cloud-canvas-command-line-resource-group-commands}
...
#### resource\-group disable {#cloud-canvas-command-line-resource-group-disable}
...
#### resource\-group enable {#cloud-canvas-command-line-resource-group-enable}
...
#### resource\-group list {#cloud-canvas-command-line-resource-group-list}
...
#### resource\-group list\-resources {#cloud-canvas-command-line-resource-group-list-resources}
...
#### resource\-group upload\-resources \(upload, update\) {#cloud-canvas-command-line-resource-group-upload}
...
### `resource-importer` Commands {#cloud-canvas-command-line-resource-importer-commands}

The `lmbr_aws resource-importer` commands list and import AWS resource definitions to your Cloud Canvas project\.

#### resource\-importer import\-resource {#cloud-canvas-command-line-resource-importer-import-resource}
...
#### resource\-importer list\-importable\-resources {#cloud-canvas-command-line-resource-importer-list-importable-resources}
