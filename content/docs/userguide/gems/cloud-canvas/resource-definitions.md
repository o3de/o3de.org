...
## Game Resource Definitions {#cloud-canvas-resource-definitions-location}
...
The default *\{game\}*`\AWS` directory contents are created by the `lmbr_aws` [project create](/docs/userguide/gems/cloud-canvas/command-line#cloud-canvas-command-line-project-create) command\.
...
### resource\-group\\*\{resource\-group\}* Directory {#cloud-canvas-resource-group-subdirectories}
...
#### The lambda\-function\-code Directory {#cloud-canvas-lambda-function-code-subdirectory}
...
#### **local\-project\-settings\.json** {#cloud-canvas-local-project-settings}
...
The `ProjectStackId` property is set by the [project create](/docs/userguide/gems/cloud-canvas/command-line#cloud-canvas-command-line-project-create) command\. If for some reason you want to associate the project with an existing project stack, you can use the AWS Management Console to look up the stack's ARN and paste it into the `local-project-settings.json` file \(navigate to AWS CloudFormation, select the stack, select **Overview**, and then copy the value of the `Stack Id` property\)\.
...
For more information, see [profile default](/docs/userguide/gems/cloud-canvas/command-line#cloud-canvas-command-line-profile-default)\.

## Project Template Files {#cloud-canvas-project-template-files}
...
### project\-template\.json {#cloud-canvas-project-template}
...
### deployment\-template\.json {#cloud-canvas-deployment-template}
...
### deployment\-access\-template\.json {#cloud-canvas-deployment-access-template}
...
#### Parameters {#cloud-canvas-deployment-access-template-parameters}
...
#### Resources {#cloud-canvas-deployment-access-template-resources}

This section describes some of the key resources that are defined in the example `deployment-access-template.json` file\.

##### Player {#cloud-canvas-deployment-access-template-player-resource}
...
##### PlayerLoginRole {#cloud-canvas-deployment-access-template-player-login-role-resource}
...
##### PlayerLoginIdentityPool {#cloud-canvas-deployment-access-template-player-login-identity-pool-resource}
...
##### PlayerAccessIdentityPool {#cloud-canvas-deployment-access-template-player-access-identity-pool-resource}
...
### Template Extension Files {#cloud-canvas-template-extension-files}
...
To create extension files, you can use the `lmbr_aws` [project create\-extension\-template](/docs/userguide/gems/cloud-canvas/command-line#cloud-canvas-command-line-project-create-extension-template) command\. To create a template file of a particular type, use the corresponding argument, as the following examples show\.
...
## Code Directories {#cloud-canvas-cgf-code-directories}
...
### The project\-code Directory {#cloud-canvas-project-code-subdirectory}
