...
{{< note >}}
Images live in subdirectories of the `/static/` directory of the O3DE documentation repository. Guide specific images are placed in subdirectories of `/static/images/`, while shared images such as logos, are placed in subdirectories of `/static/img/`. The directory structure of `/static/images/` is a mirror of the directory structure of `/docs/`. When submitting images as part of a PR, ensure the images are placed in the appropriate subdirectories.
{{< /note >}}
