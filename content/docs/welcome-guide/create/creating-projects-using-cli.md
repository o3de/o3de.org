...
1. Open a command line window and change to your O3DE engine directory.
    ...
    1. If you set up your engine as a [source engine](/docs/welcome-guide/setup/setup-from-github/#build-the-engine), use the engine source directory.

        ```cmd
        cd C:\o3de
        ```

    1. Or, if you installed O3DE or built your engine as a [SDK engine](/docs/welcome-guide/setup/setup-from-github/#build-the-engine) using the `INSTALL` target, use the installed engine directory.

        ```cmd
        cd C:\o3de-install
        ```

1. Use the `o3de` script in the `scripts` subdirectory to create a new external project. The `create-project` command, used with the `project-path` and no other options, creates a new project using the default project template. This command also registers the engine to the project in the project's `project.json` manifest.
...
Use CMake to build the Visual Studio project in the build directory of your O3DE project. For instructions, choose the tab that corresponds to your engine build type. If you installed O3DE, choose "Pre-Built SDK Engine".

{{< tabs name="Project Manager locations" >}}
{{% tab name="Source Engine" %}}

1. Build the project launcher and **O3DE Editor** using the solution you created in the `build/windows_vs2019` directory. The `profile` build configuration is shown in this example.

    ```cmd
    cmake --build build/windows_vs2019 --target MyProject.GameLauncher Editor --config profile -- /m
    ```

    When specifying `Editor` as a build target, the **Asset Processor** and **Project Manager** will be built too, since they are dependencies of the **Editor**.
...
1. When the build is complete, the engine and project binaries can be found under `build/windows_vs2019/bin/profile` in the project directory.

1. Run **O3DE Editor** to verify the project is ready to use.
...
1. The project should build within minutes. When the build is complete, the project binaries can be found under `build/windows_vs2019/bin/profile` in the project directory.
...
1. Run **O3DE Editor** to verify the project is ready to use.
    ...
    If you don't supply the project path when you run the Editor from the command line, **Project Manager** will launch. Using the Project Manager's GUI, you can edit your project's settings, add or remove Gems from the project, rebuild your project, and launch the Editor.

    {{< important >}}
If you built the engine from source using the INSTALL target, make sure you launch the Editor and other tools from the **install** directory, _not_ the build directory in the engine root. The Windows install directory will typically end in `/bin/Windows/profile`.
    {{< /important >}}
