---
linkTitle: Generate Terrain
title: Generate Terrain with a Heightmap
description: Learn to generate terrain in Open 3D Engine (O3DE) by creating entities and with Landscape Canvas.
weight: 300
toc: true
---

Terrain in Open 3D Engine (O3DE) is generated from *gradients*. A gradient, in this context, is a representation of changes in elevation. The gradient values range from dark (low elevation) to light (high elevation). Gradients can be generated procedurally or processed from an image. In this tutorial, you'll generate terrain with an image gradient, apply materials to the terrain, and add support for PhysX simulations with a heightfield collider and physics materials.
...
* Ensure the image is square with a power of two resolution (for example, 256 x 256, 512 x 512, 1024 x 1024, and so on). For the best result, each pixel in the heightmap should represent one world meter. A 1024 x 1024 heightmap, for example, represents just over one square kilometer of terrain.
...
{{< important >}}
The default mapping of a heightmap image is one pixel to one world unit in O3DE. This means that in order to see the full resolution of the heightmap as a terrain, the terrain must be as large (in world units) as the heightmap (in pixels). Even for small heightmaps, you might need to make the following temporary adjustments so that you can view the entire terrain at full resolution while you build it:

1. In the **Terrain World Renderer** level component, set both the **Mesh render distance** and the **First LOD distance** properties to a value that is greater than the resolution of the heightmap.

2. In the Camera entity, in the **Camera** component, set the **Far clip distance** property to a value that is greater than the resolution of the heightmap.

3. In the Camera entity, in the **Camera** component, choose **Be this camera** to view the level through the camera entity.

4. In the viewport camera settings in the upper-right of the viewport {{< icon "camera.svg" >}} set the **Camera Speed** to an appropriate value for the size of the terrain. `100.0` is a good reference point for large terrains.

These suggestions are meant to help you view the entire terrain while you build it in O3DE Editor. In Launcher applications, these settings should be set to reasonable values for performance considerations.
{{< /important >}}

## Terrain setup

Generating terrain requires multiple entities that reference each other. You can set up the entities and references manually, or you can use [Landscape Canvas](/docs/user-guide/gems/reference/environment/landscape-canvas) to place and connect nodes in a visual graph that generates terrain. The advantage of using Landscape Canvas is that it creates and configures entities in the level based on your node graph, and it also provides a useful visual representation of the relationships between the various entities and components that generate the terrain.
...
4. To generate terrain that is the same resolution as the heightmap, in the **Axis Aligned Box Shape** component, set the value of the **Dimensions:** **X** and **Y** properties to the resolution (in pixels) of the heightmap. The **Dimensions: Z** property can be used to scale the elevation. Enter a value for the **Dimensions: Z** property that roughly represents the maximum desired elevation in meters.
    ...
    {{< tip >}}
If the dimensions of the **Axis Aligned Box Shape** component are smaller (in world units) than the heightmap (in pixels), the resulting terrain has a lower fidelity might have sharp changes in elevation. You'll get the best results from a 1:1 mapping of heightmap pixels to terrain world units.
    {{< /tip >}}
...
5. The default minimum terrain elevation is 0, so the bottom of the box shape needs to be at least at 0 on the world Z axis, otherwise, the lower elevation values will be clipped. In the Terrain Spawner entity, in the **Transform** component, set the **Translate: Z** property to a value that is half of the value you used for the **Dimensions: Z** property in the previous step. For example, if you entered `100.0` in the previous step, use `50.0` for the **Translate: Z** property to move the entity up so that the bottom of the box is at 0 on the world Z axis.
    ...
    {{< tip >}}
Once you have set the dimensions of the box shape, you can hide it from the {{< icon "helpers.svg" >}} **Toggle viewport helpers** button in the upper-right of the viewport by unchecking **Helpers** in the list.
    {{< /tip >}}
...
1. If there is an existing terrain in your level, delete or hide the terrain entities.
...
5. In Landscape Canvas, on the **Terrain Layer Spawner** node, choose **Add Extenders** and select **Terrain Gradient List** from the list. This adds a **Terrain Gradient List** component to the terrain spawner entity that you'll use to reference the heightmap later in this tutorial.
...
The macro material is a basic material that applies to the entire terrain. It supports a simple color texture and a normal texture. The macro material is displayed on terrain that is far from the camera.

Detail materials are standard materials that you can create with [Material Editor](/docs/atom-guide/look-dev/materials/material-editor/). They are assigned to the terrain through surface tag names that are generated by gradients, similar to how this tutorial used an image gradient to create elevation. This allows you to assign many materials across a large terrain surface and blend between them. Detail materials are displayed on terrain surfaces that are close to the camera.

{{< tip >}}
In the **Terrain World Renderer** level component, there is a property group named **Detail material configuration** that you can use to configure how far from the camera to switch from detail material to macro material, and the distance over which to blend between the materials.
{{< /tip >}}
...
* Magenta - This is th macro material. It's displayed on terrain that is 512 units from the camera. The blend distance between this macro material and the detail materials is 54 units.

* Green - This is the default detail material. Detail materials are layered, and the default material is the base layer. The default material is displayed on areas that aren't aren't assigned a detail material through a surface tag name, but are close to the camera.
...
{{< note >}}
If no detail materials are applied to the terrain, the macro material is also displayed on terrain surfaces that are close to the camera.
{{< /note >}}
